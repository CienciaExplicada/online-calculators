...
for(var i=1;i<=8;i++)
{
	var nam     = "val_pr_acert_desc"+i;
	var nam_des = "val_num_desc"+i;
	
	var nams     = "slider_pr_acert_desc"+i;
	var nams_des = "slider_desc"+i;
	
	var def_pr  = (i<5 ? 75 : (i<8 ? 67 : 50));
	var def_des = 1; //(i<5 ? 1 : (i<8 ? 67 : 50));
	
	document.write("<tr><td>Prob. good discard P"+i+": <span id=\""+nam+"\">"+def_pr+"</span>% &nbsp;&nbsp; </td><td><div style=\"width:150px;\"><div id=\""+nams+"\" ></div></td> <td>&nbsp; &nbsp; Discarded answers: <span id=\""+nam_des+"\">"+def_des+"</span> &nbsp;&nbsp;&nbsp; </td> <td> <div style=\"width:70px;\"> <div id=\""+nams_des+"\" ></div></div> </td> </tr>");
...
document.write('<table>'+
'<tr>'+
'<td> <div id="placeholder_mean" style="width:300px;height:200px"></div> </td>'+
'<td> <div id="placeholder_stats" style="width:300px;height:200px"></div> </td>'+
'</table>');
	
document.write('<div align="center"><p><h3>Histograms of the expected number of bundles at each stage [Q0:start, Q1-Q8:after question 1-8]</h3></p></div>');

for(var i=0;i<=8;i++)
{			
	var nam = "placeholder_t"+i;
...
function getObjInnerText(obj) 
{
	...
	else 
	{
		...
		else alert("Error: This application does not support your browser.  Try again using Chrome or Firefox.");
	}
}
...
function regenerate_all_plots() 
{
	...
	{
		var plot_mean = $.plot( placeholder_mean,
			   [ 
				{ 
					data: MEANs, 
					label: "Expected # bundles",
					lines: { show: true },
					points: { show: true }
				}, 
				{ 
		...
		$(placeholder_mean).bind("plothover", function (event, pos, item) {
			if (item) {
	...
	// Draw more stats:
	{
		var plot_stats = $.plot( placeholder_stats,
			   [ 
				{ 
					data: PR_LOSE, 
					label: "Prob. losing all",
					bars: { show: true }
				} 
				],
			   {
		...
		$(placeholder_stats).bind("plothover", function (event, pos, item) {
			if (item) {
				if (previousPoint != item.dataIndex) {
					previousPoint = item.dataIndex;
					$("#tooltip").remove();
					var x = item.datapoint[0].toFixed(2),
						y = item.datapoint[1].toFixed(4);
					showTooltip(item.pageX, item.pageY+45, "Q"+Math.floor(x)+": "+(100*y).toFixed(2)+"%");
				}
			}
			else {
				$("#tooltip").remove();
